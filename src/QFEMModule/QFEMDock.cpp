...
#include "src/QBEM/BData.h"
#include "src/QBEM/TBEMData.h"
#include "src/Globals.h"
#include "src/FloatDelegate.h"
	...
	: ScrolledDock (title, parent, flags)
{
    ...
    m_pctrlPerspective = new QPushButton(tr("Perspective View"));
    m_pctrlPerspective->setCheckable(true);
    m_pctrlPerspective->setFlat(true);
    m_pctrlSurfaces = new QPushButton(tr("Bottom Surface"));
    m_pctrlSurfaces->setCheckable(true);
    m_pctrlSurfaces->setFlat(true);
    m_pctrlTopSurface = new QPushButton(tr("Top Surface"));
    m_pctrlTopSurface->setCheckable(true);
    m_pctrlTopSurface->setFlat(true);
    m_pctrlOutline = new QPushButton(tr("Outlines"));
    ...
    m_pctrlRotor = new QPushButton(tr("Show Rotor"));
    m_pctrlRotor->setCheckable(true);
    m_pctrlRotor->setFlat(true);
    m_pctrlInternal = new QPushButton(tr("Internal Structure"));
    m_pctrlInternal->setCheckable(true);
    m_pctrlInternal->setFlat(true);
...
//------------------2DViewLayout------------------------//
    ...
    connect(m_curveStyleBox->m_showHighlightCheckBox, SIGNAL(stateChanged(int)), m_module, SLOT(OnSelChangeBladeStructure()));
...
//    m_pctrlAllSections->setMaximumWidth(180);
    ...
    m_pctrlshellColor    = new NewColorButton;
    m_pctrlsparColor    = new NewColorButton;
...
//    m_pctrlStructureTable->setFixedWidth(520);
    ...
    m_infoLabel = new QLabel(tr("Import Loading from BEM Simulation at a choosen TSR"));

    m_BEMGroupBox = new QGroupBox (tr("Simulation"));
    m_BEMDataBox = new BEMDataComboBox (&g_bemdataStore);
    m_BEMDataBox->setParentBox(m_module->m_QFEMToolBar->m_rotorComboBox);
    m_BEMDataBox->setSizeAdjustPolicy(QComboBox::AdjustToContents);
    m_BEMDataBox->setMinimumWidth(170);
    connect (m_BEMDataBox, SIGNAL(valueChangedVoid()), this, SLOT(OnFillTSR()));
    grid = new QGridLayout ();
    grid->addWidget(m_BEMDataBox, 0, 0);
    m_BEMGroupBox->setLayout(grid);

    m_PropBEMDataBox = new BEMDataComboBox (&g_propbemdataStore);
    m_PropBEMDataBox->setParentBox(m_module->m_QFEMToolBar->m_rotorComboBox);
    m_PropBEMDataBox->setSizeAdjustPolicy(QComboBox::AdjustToContents);
    m_PropBEMDataBox->setMinimumWidth(170);
    connect (m_PropBEMDataBox, SIGNAL(valueChangedVoid()), this, SLOT(OnFillTSR()));
    grid->addWidget(m_PropBEMDataBox, 0, 1);

    ImportLoadingLayout->addWidget(m_BEMGroupBox);

    m_TSRGroupBox = new QGroupBox (tr("TSR"));
    m_TSRBox = new QComboBox;
    grid = new QGridLayout ();
    grid->addWidget(m_TSRBox, 0, 0);
    m_TSRGroupBox->setLayout(grid);
    ImportLoadingLayout->addWidget(m_TSRGroupBox);
...
//    m_LoadingTable->setFixedWidth(520);
    ...
    LoadingEditLayout->addWidget(m_infoLabel);
...
void QFEMDock::OnTabChanged (int i) {
    if (i == 1) {
        ...
        m_module->isLoadingView = true;
        m_module->isStructView = false;
...
void QFEMDock::OnImportLoading(){
    ...
    if (m_TSRBox->count()){
        ...
        if (g_mainFrame->m_currentMode == PROP_MODE)
            bdata = m_PropBEMDataBox->currentObject()->GetBData().at(m_TSRBox->currentIndex());
        else
            bdata = m_BEMDataBox->currentObject()->GetBData().at(m_TSRBox->currentIndex());
...
void QFEMDock::OnFillTSR(){

    m_TSRBox->clear();

    if (g_mainFrame->m_currentMode == PROP_MODE){
        if (m_PropBEMDataBox->currentObject()){
            BEMData *bem = m_PropBEMDataBox->currentObject();
            for (int i=0; i< bem->GetBData().size();i++){

                m_TSRBox->addItem(bem->GetBData().at(i)->GetTSR());
            }
        }
    }
    else{
        if (m_BEMDataBox->currentObject()){
            BEMData *bem = m_BEMDataBox->currentObject();
            for (int i=0; i< bem->GetBData().size();i++){
    ...
    if (m_TSRBox->count()) m_TSRBox->setEnabled(true);
    else m_TSRBox->setEnabled(false);
}
...
//    m_pctrlStructureTableView->setMinimumWidth(0.9*this->width());
    ...
    if (m_module->m_structure){
        ...
        {
            int unitwidth = (int)((double)(m_pctrlStructureTableView->width()-45)/4.0);
...
//    m_LoadingTable->setMinimumWidth(0.9*this->width());
    int unitwidth = (int)(((double)m_LoadingTable->width()-45)/3.0);
...
//    m_pctrlStructureTable->setMinimumWidth(0.9*this->width());
    if(m_module->m_structure){
        if (m_module->m_structure->StructType == 0)
        {
            int unitwidth = (int)(((double)m_pctrlStructureTable->width()-45)/4.0);
...
//    m_LoadingTableView->setMinimumWidth(0.9*this->width());
    unitwidth = (int)(((double)m_LoadingTableView->width()-45)/3.0);
...
void QFEMDock::InitStructureTable()
{
    ...
    if (m_module->m_structure->AbsoluteShell) m_pStructModel->setHeaderData(0, Qt::Horizontal, tr("Shell Thickness [m]"));
    else m_pStructModel->setHeaderData(0, Qt::Horizontal, tr("Shell Thickness(%)"));

    if (m_module->m_structure->AbsoluteSpar) m_pStructModel->setHeaderData(1, Qt::Horizontal, tr("Spar Thickness [m]"));
...
bool QFEMDock::InitLoadingDialog(BladeStructureLoading *pLoading){
    ...
    m_pLoadingModel->setHeaderData(0, Qt::Horizontal, tr("Radial Position [m]"));
    m_pLoadingModel->setHeaderData(1, Qt::Horizontal, tr("Normal Loading [N]"));
    m_pLoadingModel->setHeaderData(2, Qt::Horizontal, tr("Tangential Loading [N]"));
    ...
    m_pFloatDelegate = new FloatDelegate(this);
...
void QFEMDock::OnNewLoading(){
    ...
    g_QFEMModule->ObjectIsEdited = true;
    ...
    QString newname = g_bladestructureloadingStore.createUniqueName(m_module->m_structure->getName()+" Loading Data");
...
void QFEMDock::OnSaveLoading(){
    ...
    g_QFEMModule->ObjectIsEdited = false;
...
void QFEMDock::OnNew() {
	if (m_module->m_rotor) {
        ...
        QString newname = g_bladeStructureStore.createUniqueName(m_module->m_rotor->getName()+" Structural Model");
...
void QFEMDock::onLineButtonClicked() {
    ...
    if (m_curveStyleBox->GetLinePen(pen))
        m_module->m_structure->setPen(pen);
}

void QFEMDock::onShowCheckBoxCanged () {
    m_module->m_structure->setShownInGraph(m_curveStyleBox->m_showCheckBox->isChecked());
	m_module->reloadAllGraphCurves();
}

void QFEMDock::onShowPointsCheckBoxCanged () {
    m_module->m_structure->setDrawPoints(m_curveStyleBox->m_showPointsCheckBox->isChecked());
    m_module->update();
}

void QFEMDock::onShowCurveCheckBoxCanged () {
    m_module->m_structure->setDrawCurve(m_curveStyleBox->m_showCurveCheckBox->isChecked());
    m_module->update();
}

void QFEMDock::SetShownBladeStructure(BladeStructure *structure){

    for (int i=0;i<g_bladeStructureStore.size();i++) g_bladeStructureStore.at(i)->setHighlight(false);
    if (m_curveStyleBox->m_showHighlightCheckBox->isChecked() && structure)
        structure->setHighlight(true);

    m_curveStyleBox->UpdateContent(structure);
}
