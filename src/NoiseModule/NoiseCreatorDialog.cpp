	  ...
	  m_opPointViewWidget(NULL)
{
	...
	tabWidget->addTab(widget, "Op. Points");
		...
		widget->setLayout(hBox);
            groupBox = new QGroupBox ("OpPoints");
			groupBox->setMinimumHeight(300);
			hBox->addWidget(groupBox);
				QGridLayout *grid = new QGridLayout;
				grid->setColumnStretch(5, 1);
				groupBox->setLayout(grid);
                    QLabel *label = new QLabel ("Select OpPoints from");
                    ...
                    connect(m_polarComboBox, SIGNAL(currentIndexChanged(int)), this, SLOT(onPolarBoxChange()));
		...
		if (m_editedSimulation->getSelectFrom() == NoiseParameter::OnePolar) {
			if (!m_editedSimulation->getAnalyzedOpPoints().isEmpty()) {
				m_airfoilComboBox->setCurrentObject(
						static_cast<Airfoil*>(m_editedSimulation->getAnalyzedOpPoints()[0]->getParent()->getParent()));
				m_polarComboBox->setCurrentObject(
						static_cast<Polar*>(m_editedSimulation->getAnalyzedOpPoints()[0]->getParent()));
			}
		}
		
        QVector<OperationalPoint*> analyzedOpPoints = m_editedSimulation->getAnalyzedOpPoints();
...
void NoiseCreatorDialog::prepareOpPointRecords(bool allPolars) {
    ...
    for (int i = 0; i < g_operationalPointStore.size(); ++i) {
        OperationalPoint *opPoint = g_operationalPointStore.at(i);
		if (allPolars || static_cast<Polar*>(opPoint->getParent()) == m_polarComboBox->currentObject()) {
...
void NoiseCreatorDialog::onCreateButtonClicked() {
    ...
    QList<OperationalPoint*> analyzedOpPoints;
	for (int i = 0; i < m_opPointRecords.size(); ++i) {
